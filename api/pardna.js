...
async function q(method, table, params = {}, body = null) {
  let url = BASE + table;
    ...
    switch (action) {
      ...
      case 'load': {
        ...
        result = {
          members: members || [],
          payments: payments || [],
          adminSettings: Array.isArray(adminArr) ? adminArr[0] : adminArr,
          borrows: borrows || [],
          donations: donations || [],
      ...
      case 'insertExtraCapital': {
        const r3 = await q('POST', 'extra_capital', {}, {member_id:p.memberId, week_num:p.weekNum, amount:p.amount, note:p.note||''});
        result = {data: Array.isArray(r3)?r3[0]:r3};
        break;
      }

      case 'reviewRequest': {
        await q('PATCH', 'borrow_requests', {id:`eq.${p.reqId}`}, {status:p.status, reviewed_on:new Date().toISOString().split('T')[0]});
        if (p.status === 'approved') {
          const r4 = await q('POST', 'borrows', {}, {member_id:p.memberId, amount:p.amount});
          result = {data: Array.isArray(r4)?r4[0]:r4};
        } else {
          result = {ok:true};
        }
        break;
      ...
      case 'upsertDonation':
        if (p.existingId) {
          ...
          const r5 = await q('POST', 'donations', {}, {member_id:p.memberId, amount:p.amount});
          result = {data: Array.isArray(r5)?r5[0]:r5};
        } else {
          result = {ok:true};
        }
        break;
      ...
      case 'insertBorrowRequest': {
        const r6 = await q('POST', 'borrow_requests', {}, {member_id:p.memberId, amount:p.amount, reason:p.reason||''});
        result = {data: Array.isArray(r6)?r6[0]:r6};
        break;
      }
